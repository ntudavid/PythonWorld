...
layer1 = add_layer(xs, 784, 20, activation_function = tf.nn.tanh) #activation_function = tf.nn.tanh is needed, or failed
results = add_layer(layer1, 20, 10, activation_function = tf.nn.softmax)
...
model = tf.train.GradientDescentOptimizer(0.8).minimize(cross_entropy)
...
for i in range(10001):
    ...
    sess.run(model,feed_dict = {xs:batch_xs, ys:batch_ys, keep_prob:0.8})
    if(i%1000==0):
        print(compute_accuracy(mnist.test.images, mnist.test.labels,0.8))
